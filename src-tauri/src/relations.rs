...
pub async fn create_relation(
    ...
    let relation = sqlx::query_as::<_, types::NewRelation>("INSERT INTO relations (uuid, description , source_asset_uuid, target_asset_uuid ) VALUES (?1, ?2, ?3, ?4) RETURNING * ").bind(uuid).bind(description).bind(source_asset_uuid).bind(target_asset_uuid).fetch_one(pool).await.map_err(|e| e.to_string())?;
    Ok(relation)
}
